...
def test_model_calls_wait_at_the_configured_gate():
    ...
    one gateway, and the degraded gateway that followed became the pipeline's largest single loss:
    forty-four of seventy package attempts refused with `the gateway did not answer: timed out`
    while a one-word prompt sent by hand came back in three seconds.
    """
    import threading
    ...
    inside, released, entered = threading.Event(), threading.Event(), threading.Event()
