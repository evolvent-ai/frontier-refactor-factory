...
def test_a_subject_can_be_served_over_the_wire_inside_the_sandbox(remote):
    """The seam, in the place tasks are actually graded.

    This is the assertion that ties the two halves of the factory together: the shim table is data
    that has only ever been exercised on this host, and the wire is what a freeze runs over. If a
    shim works here and not in the sandbox, every Expectation would be frozen against a subject the
    ...
    with open(os.path.join(local, shim.template), "w") as handle:
        handle.write(shims.source(shim))

    room = "/home/user/frf-serve"
    remote.push(local, room)

    # Two probes and a refusal, fed on stdin exactly as the runner would. The requests are written
    # into the sandbox as a file rather than echoed by the shell: quoting JSON through two layers of
    # shell is how a test ends up debugging its own quoting instead of the thing it is testing.
    import json
    requests = "".join(json.dumps(r) + "\n" for r in (
        {"id": 1, "op": "run", "call": "entry", "args": [[1, 2, 3]]},
        {"id": 2, "op": "run", "call": "entry", "args": [[]]},
        {"id": 3, "op": "time", "call": "entry", "args": [[1, 2]], "repeats": 20},
    ))
    with open(os.path.join(local, "requests.jsonl"), "w") as handle:
        handle.write(requests)
    remote.push(local, room)

    served = remote.run(
        ["sh", "-c", "python3 %s subject.py < requests.jsonl" % shim.template], workdir=room)
    assert served.ok, served.tail()

    lines = [line for line in served.stdout.splitlines() if line.strip()]
    assert len(lines) == 3, served.stdout

    first, refusal, timed = (json.loads(line) for line in lines)
    assert first["ok"] and first["value"] == 6
    assert not refusal["ok"] and "empty" in refusal["error"]
    # A refusal did not kill the process: the third request was still answered.
    assert timed["ok"] and timed["seconds"] > 0
